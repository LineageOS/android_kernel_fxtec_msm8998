...
#define AS6313_SLAVE_ADDR 0x0c
#define AS6313_CLK_GPIO   32  // should change to board's real GPIO
#define AS6313_DAT_GPIO   33  // should change to board's real GPIO

struct as6313_data {
	unsigned int sck_gpio;
	unsigned int sda_gpio;
	unsigned int sck_level;
	unsigned int sda_level;
	unsigned int flags;
	int mode;
	spinlock_t  lock;
};
...
static void set_clkpin_output(int value) { // set clk gpio pin direction = out
    gpio_direction_output(as6313_priv->sck_gpio, value);
}

static void set_datpin_output(int value) { // set dat gpio pin direction = output 
    gpio_direction_output(as6313_priv->sda_gpio, value);
}

static void set_datpin_input(void) { // set dat gpio pin direction = input
    gpio_direction_input(as6313_priv->sda_gpio);
}

int get_dat(void) {   //read dat gpio value
    return gpio_get_value(as6313_priv->sda_gpio);
}
...
static void bitbang(int clk, int dat, int flag){
    ...
    as6313_priv->sda_level = dat;
    as6313_priv->sck_level = clk;
...
static void io_write_byte(int data) {
   int i;
   bitbang(0,as6313_priv->sda_level,GPIOF_DIR_OUT);  //CLK = 0;
   ...
   for (i=0;i<8;i++) {
		...
		bitbang(1,as6313_priv->sda_level,GPIOF_DIR_OUT);
        bitbang(0,as6313_priv->sda_level,GPIOF_DIR_OUT);                           // CLK ==>1 ==>0
   }
   bitbang(0,as6313_priv->sda_level,GPIOF_DIR_IN);  //get ACK
   bitbang(1,as6313_priv->sda_level,GPIOF_DIR_IN);  //get ACK
}
...
{
   int i, data;
   bitbang(0,as6313_priv->sda_level,GPIOF_DIR_IN);  //CLK = 0; output disabled
   data=0;
   for (i = 0; i < 8; i++) {
       bitbang(1,as6313_priv->sda_level,GPIOF_DIR_IN);
       if(test_dat())
           data |=1;
       if(i<7)
           data<<=1;
       bitbang(0,as6313_priv->sda_level,GPIOF_DIR_IN);
    }
...
void as6313_USB(void) { io_write_reg0(0); }   //back to USB mode
...
{
	pr_info("as6313_set_mode mode=%d\n", mode);
	switch (mode) {
		case AS_MODE_USB:
			as6313_USB();
			break;
...
EXPORT_SYMBOL_GPL(as6313_set_mode);
...
static int as6313_get_mode(void)
{
	...
	pr_info("as6313_get_mode value=%d\n", value);
	if (value > -1 && value <= 7) {
		int mode = value & 0x07;
		switch (mode) {
			case 0x00:
	...
	pr_info("as6313_get_mode mode=%d\n", as6313_priv->mode);
                ...
                unsigned long arg)
{
	...
	switch(cmd) {
		...
		case AS_IOCTL_GET_MODE:
			...
			pr_err("%s: copy_to_user for AS_IOCTL_GET_MODE failed\n",
				__func__);
			break;
...
static int as6313_gpio_init(struct device *dev)
{
	...
	as6313_priv->sck_gpio = of_get_named_gpio(dev->of_node, "sck-gpio", 0);
	pr_info("as6313_priv->sck_gpio =%d\n",as6313_priv->sck_gpio);
	if (as6313_priv->sck_gpio < 0) {		
		pr_err("Looking up sck-gpio property in node %s failed %d\n",				
			dev->of_node->full_name, as6313_priv->sck_gpio);	
		goto gpio_init_err;	
	}	
	err = gpio_request_one(as6313_priv->sck_gpio, GPIOF_OUT_INIT_HIGH, NULL);
	pr_info("request gpio%d, ret=%d\n", as6313_priv->sck_gpio, err);
	if (err) {
		pr_err("[%s] request gpio%d failed\n", __FUNCTION__, 
			as6313_priv->sck_gpio);
	}
	
	as6313_priv->sda_gpio = of_get_named_gpio(dev->of_node, "sda-gpio", 0);
	pr_info("as6313_priv->sck_gpio =%d\n",as6313_priv->sda_gpio);
	if (as6313_priv->sda_gpio < 0) {		
		pr_err("Looking up sck-gpio property in node %s failed %d\n",				
			dev->of_node->full_name, as6313_priv->sda_gpio);	
		goto gpio_init_err;	
	}
	
	err = gpio_request_one(as6313_priv->sda_gpio, GPIOF_OUT_INIT_HIGH, NULL);
	pr_info("request gpio%d, ret=%d\n", as6313_priv->sda_gpio, err);
	if (err) {
		pr_err("[%s] request gpio%d failed\n", __FUNCTION__, 
			as6313_priv->sda_gpio);
	}
	
	as6313_priv->sck_level = 1;
	as6313_priv->sda_level = 1;
...
AS6313_DEVICE_SHOW_SET(mode, AS_DBG_TYPE_MODE);
AS6313_DEVICE_SHOW_SET(irq, AS_DBG_TYPE_IRQ);
AS6313_DEVICE_SHOW_SET(reg0, AS_DBG_TYPE_REG0);
AS6313_DEVICE_SHOW_SET(reg1, AS_DBG_TYPE_REG1);
...
static int as6313_probe(struct platform_device *pdev) {
	int err;
	//int i=0;
	pr_err("%s: register as6313 driver init\n", __func__);
	...
	if (err < 0){
		pr_info("%s sysfs_create_group fail\n",__func__);
		goto exit_init;
	}
...
static int as6313_remove(struct platform_device *pdev) {
	...
	gpio_free(as6313_priv->sck_gpio);
	gpio_free(as6313_priv->sda_gpio);
	kfree(as6313_priv);
	return 0;
}
