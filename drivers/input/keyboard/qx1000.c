...
/* Extra 'hold down' interval to prevent duplicate key events */
	...
	for (key_nr = 0; key_nr < AW9523_NR_KEYS; ++key_nr) {
		...
		if (key_state && !pressed[key_nr]) {
			...
			if (key_nr == last_released &&
				...
				printk(KERN_INFO "aw9523b: extra debounce: press ignored");
				release_to_ignore = key_nr;
				continue;
			}
			...
			/* Extra debounce check */
			if (key_nr == release_to_ignore) {
				printk(KERN_DEBUG "aw9523b: extra debounce: release ignored");
				release_to_ignore = -1;
				continue;
			}
