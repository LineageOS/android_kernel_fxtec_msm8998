...
static const enum idea_key map_idea_key(const char *label)
{
        if (!strcmp(label, "shift"))
                return IDEA_SHIFT;
        if (!strcmp(label, "ctrl"))
                return IDEA_CTRL;
        if (!strcmp(label, "fn_l"))
                return IDEA_FN_L;
        if (!strcmp(label, "home"))
                return IDEA_HOME;
        if (!strcmp(label, "alt"))
                return IDEA_ALT;
        if (!strcmp(label, "fn_r"))
                return IDEA_FN_R;
        return IDEA_NONE;
};
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	int state;
	u16 mask = 0;
	enum idea_key key = map_idea_key((button->desc ? button->desc : "(null)"));
	u16 keycode = (g_physical_modifiers & KF_FN ? mod_array[key] : mod_fn_array[key]);
	bool report = true;
