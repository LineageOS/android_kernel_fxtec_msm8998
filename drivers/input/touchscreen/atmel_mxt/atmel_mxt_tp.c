...
/* Regulator voltages */
#define MXT_VDDIO_MIN_UV	1808000
#define MXT_VDDIO_MAX_UV	1808000
#define MXT_VDD_MIN_UV		3008000 //2800000
#define MXT_VDD_MAX_UV		3008000 //2800000
...
retry_probe:
	...
	if (error) {
		...
		if (error) {
			...
			if (++retry_count > 10) {
 				dev_err(&client->dev, "Could not recover device from bootloader mode and will force update.\n");
				/*Mark the bootloader state and update later using the shell command*/
...
static int mxt_load_fw(struct device *dev, const char *fn)
{
	...
	if (ret) {
		...
		if (ret) {
			dev_err(dev, "mxt_check_bootloader failed, will force update.\n");
		}
	} else {
    ...
    dev_info(dev, "fw_step 1: finished");
...
release_firmware:
	if(data->state == FAILED){
		dev_err(dev, "data->state == FAILED return\n");
	}
	...
	size_t count, bool *upgraded)
{
	...
	if(data->state == BOOTLOADER){
		dev_info(dev, "mxt device is under bootloader mode,%s will force update\n", fw_name);
		goto force_update;
	}
...
force_update:
	if (count <= 0){
		dev_err(dev, "count is invalid. (count = %zd)\n", count);
		return -EINVAL;
	}
