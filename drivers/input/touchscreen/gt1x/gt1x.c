...
static int gt1x_parse_dt(struct device *dev)
{
    ...
    if (!gpio_is_valid(gt1x_int_gpio) || !gpio_is_valid(gt1x_rst_gpio)||!gpio_is_valid(gt1x_vdd_gpio)) {
        GTP_ERROR("Invalid GPIO, irq-gpio:%d, rst-gpio:%d, vdd-gpio:%d",
            gt1x_int_gpio, gt1x_rst_gpio, gt1x_vdd_gpio);
        return -EINVAL;
    }
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//if(1==tp_ilitek_flag) return -ENODEV;
