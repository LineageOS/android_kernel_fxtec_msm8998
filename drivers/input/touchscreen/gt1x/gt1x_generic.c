...
s32 gt1x_reset_guitar(void)
{
	...
	if (ret < 0) {
		GTP_DEBUG("gt1x_reset_guitar GTP works in polling mode.");
	} else {
		GTP_DEBUG("gt1x_reset_guitar GTP works in interrupt mode.");
	}
...
#endif
		{
			free_irq(gt1x_i2c_client->irq, gt1x_i2c_client); //zds add for test		
    		...
    		GTP_GPIO_AS_INT(GTP_INT_PORT);
            flag = 1;
			gt1x_request_irq();  //zds add for test
#if GTP_ESD_PROTECT
            ret = gt1x_set_reset_status();
#else
            ret = gt1x_i2c_test();
