...
int fts_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
    int ret = 0;
    int i = 0;
	//FTS_ERROR("zds fts_i2c_read() enter!!!!");
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	// sprintf(buf, "%s\n", writebuf);
	//FTS_ERROR("zds fts_i2c_write() enter!!!!client->addr =%d,buf=%s,len=%d",client->addr,writebuf,writelen);
	//puts(writebuf);
...
{
    u8 buf[2] = {0};
//FTS_ERROR("zds fts_i2c_write_reg() enter!!!!");
    buf[0] = regaddr;
    buf[1] = regvalue;
