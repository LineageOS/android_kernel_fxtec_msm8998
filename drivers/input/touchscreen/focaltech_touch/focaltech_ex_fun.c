...
#if FTS_PSENSOR_EN
extern bool proximity_status;
//extern int proximity_status_flag;
int in_call_state=0;
...
static ssize_t fts_in_call_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int err;
	int ret;
	unsigned long value;
	...
	err = kstrtoul(buf, 10, &value);
    //in_call_state = value;
	//printk("[fts]%s: value(%ld)\n", __func__, value);
	//fts_ts_resume(&client->dev);
	if(1 == value){
			in_call_state++;
			//printk("[fts]%s: in_call_state(%d)\n", __func__, in_call_state);
	}else if((0 == value)&&(in_call_state >= 1)){
			in_call_state--;
			//printk("[fts]%s: in_call_state2(%d)\n", __func__, in_call_state);
	}

	if(0 < in_call_state){
		ret = fts_i2c_write_reg(client, FTS_REG_FACE_DEC_MODE_EN, 0x01);
		//printk("[fts]%s: ret(%d)\n", __func__, ret);
	}else{
		ret = fts_i2c_write_reg(client, FTS_REG_FACE_DEC_MODE_EN, 0x00);
		//printk("[fts]%s: ret2(%d)\n", __func__, ret);
	}
...
static ssize_t fts_in_call_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
#if FTS_PSENSOR_EN
static DEVICE_ATTR(fts_proximity, S_IRUGO | S_IWUSR, fts_proximity_show, fts_proximity_store);
static DEVICE_ATTR(fts_in_call, S_IRUGO | S_IWUSR, fts_in_call_show, fts_in_call_store);
#endif
...
#if FTS_PSENSOR_EN    
    &dev_attr_fts_proximity.attr,
    &dev_attr_fts_in_call.attr,
#endif    
    NULL
};
