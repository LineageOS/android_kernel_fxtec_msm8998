...
static unsigned int MA_DRV_VERSION_DEV = 0x00004006;
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd){
        ...
        case MA_IOC_GVER:
			...
			ret = copy_to_user((unsigned int*)arg, &MA_DRV_VERSION_DEV, sizeof(unsigned int));
            mutex_unlock(&ioctl_lock);
            break;
		case SCREEN_ON:
        ...
        case SET_SPI_SPEED:
            ...
            //kingsun/ly,modify for A072N fingerprint
